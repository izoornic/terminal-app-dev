<div class="mt-2 bg-green-100 border-t-4 border-green-500 rounded-b text-green-900 px-4 py-3 shadow-md mb-6" role="alert">
   
    @if(!$vrsta_lokacije)
        <div class="my-2 flex justify-between">
            @foreach (App\Models\BlokacijaTip::whereIn('id', $tipovi_lokacija)->get() as $value ) {{-- getAll() as $key => $value) --}}
                <button wire:click="vrstaLokacije({{$value->id}})" class="flex mt-1 -mb-1 bg-yellow-50 text-sm text-gray-700 uppercase border border-green-500 rounded-md p-1.5 hover:bg-gray-700 hover:text-white" >
                     @switch($value->id)
                        @case(1)
                            <x-heroicon-o-wrench-screwdriver class="text-red-400 w-4 h-4" />
                        @break
                        @case(2)
                            <x-icon-warehouse class="fill-gray-400 w-5 h-5"/>
                        @break
                        @case(3)
                            <x-heroicon-o-building-library class="text-sky-400 w-5 h-5"/>
                        @break
                    @endswitch
                    <span class="ml-2">{{ $value->bl_tip_naziv }}</span>
                </button>
            @endforeach
        </div>
    @else
        <div class="mt-2">
            <p class="font-bold">{{ App\Models\BlokacijaTip::getAll()[$vrsta_lokacije] }}</p>
        </div>
        <div class="mt-2">
            <table class="min-w-full divide-y divide-gray-200 mt-4" style="width: 100% !important">
                <thead>
                    <!-- search row -->
                    <tr class="bg-orange-50">
                        <td class="p-1">
                            <x-jet-input wire:model.live="searchPLokacijaNaziv" id="" class="block bg-orange-50 w-full" type="text" placeholder="Naziv" />
                        </td>
                        <td class="p-1">
                            <x-jet-input wire:model.live="searchPlokacijaMesto" id="" class="block bg-orange-50 w-full" type="text" placeholder="Mesto" />
                        </td>
                        <td class="p-1">
                            @if($role_region['role'] == 'admin' || $role_region['role'] == 'programer')
                                <select wire:model.live="searchPlokacijaRegion" id="" class="block appearance-none bg-orange-50 w-full border border-1 border-gray-300 rounded-md text-gray-700 py-3 px-4 pr-8 round leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                        <option value="">---</option>
                                        @foreach (App\Models\BankomatRegion::getAll() as $key => $value)    
                                            <option value="{{ $key }}">{{ $value }}</option>
                                        @endforeach
                                </select>
                            @endif
                        </td>
                    </tr>
                </thead>
                <tbody class="bg-yellow-50 divide-y divide-gray-200"> 
                    @foreach ($this->lokacijeTipa($vrsta_lokacije) as $value)
                        <tr class="hover:bg-gray-100" wire:click="novaLokacija({{ $value->id }})" >   
                                <td class="p-1">
                                    @if($value->is_duplicate)<span class="text-red-500">*</span>@endif
                                    {{ $value->bl_naziv }}
                                    @if($value->is_duplicate)<br />{{ $value->bl_naziv_sufix }}@endif
                                </td>
                                <td class="p-1">{{ $value->bl_adresa }}</td>
                                <td class="p-1">
                                    {{ $value->bl_mesto}}<br /><span class="text-sm text-red-400"> {{ $value->r_naziv}}</span>
                                </td>
                        </tr>
                    @endforeach
                </tbody>
            <table>
            
            <div class="mt-4">
                {{ $this->lokacijeTipa($vrsta_lokacije)->links() }}
            </div>
        </div>
    @endif
</div>
