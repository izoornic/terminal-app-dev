<div class="p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200" style="width: 100% !important">
                        <thead>
                            <tr>
                                ...
                                <th colspan="2" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">PIB
                                </th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-2 py-2 flex">
                                                @if($item->latitude != '' && $item->longitude != '') 
                                                    <a href="{{ App\Ivan\HelperFunctions::createGmapLink($item->latitude, $item->longitude) }}" target="_blank"> <svg class="fill-sky-800 w-4 h-4 mx-auto mt-2 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M408 120C408 174.6 334.9 271.9 302.8 311.1C295.1 321.6 280.9 321.6 273.2 311.1C241.1 271.9 168 174.6 168 120C168 53.73 221.7 0 288 0C354.3 0 408 53.73 408 120zM288 152C310.1 152 328 134.1 328 112C328 89.91 310.1 72 288 72C265.9 72 248 89.91 248 112C248 134.1 265.9 152 288 152zM425.6 179.8C426.1 178.6 426.6 177.4 427.1 176.1L543.1 129.7C558.9 123.4 576 135 576 152V422.8C576 432.6 570 441.4 560.9 445.1L416 503V200.4C419.5 193.5 422.7 186.7 425.6 179.8zM150.4 179.8C153.3 186.7 156.5 193.5 160 200.4V451.8L32.91 502.7C17.15 508.1 0 497.4 0 480.4V209.6C0 199.8 5.975 190.1 15.09 187.3L137.6 138.3C140 152.5 144.9 166.6 150.4 179.8H150.4zM327.8 331.1C341.7 314.6 363.5 286.3 384 255V504.3L192 449.4V255C212.5 286.3 234.3 314.6 248.2 331.1C268.7 357.6 307.3 357.6 327.8 331.1L327.8 331.1z"/></svg></a> 
                                                @endif
    ...
    <x-jet-dialog-modal wire:model="modalBlacklistVisible">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('modalBlacklistVisible')" wire:loading.attr="disabled">
                {{ __('Otkaži') }}
            </x-jet-secondary-button>
            @if($canBlacklist)
                ...
                <div class="bg-sky-100 border-t-4 border-sky-500 rounded-b text-sky-900 px-4 py-3 shadow-md mb-6" role="alert">
                    <div class="flex">
                        <div class="py-1"><svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2H168.3zM192 256C227.3 256 256 227.3 256 192C256 156.7 227.3 128 192 128C156.7 128 128 156.7 128 192C128 227.3 156.7 256 192 256z"/></svg></div>
                            <div>
                                <p><span class="font-bold">{{ $odabranaLokacija->l_naziv}}&nbsp;{{ $odabranaLokacija->l_naziv_sufix }}</span></p>
                                ...
                                <p>e-mail: <span class="font-bold">{{ $odabranaLokacija->email }}</span>
                            </div>
                    </div>
                </div> 
            @endif
            @if($deletePosible)
                {{ __('Lokacija nije povezana sa terminalima ili korisnicima.') }}
            @else 
                    ...
                    @foreach($terminaliList as $termin)
                        <div class="bg-sky-100 border-t-4 border-sky-500 rounded-b text-sky-900 px-4 py-3 shadow-md mb-6" role="alert">
                            ...
                            <div class="text-right pt-2">
                                 @if($termin->blacklist == 1)
                                    <x-jet-secondary-button class="ml-2" wire:click="BlacklistShowModal({{$termin->tlid}})" wire:loading.attr="disabled">
                                        {{ __('Ukloni sa Blackliste') }}
                                    </x-jet-secondary-button>
                                @else
                                    <x-jet-button class="ml-2" wire:click="BlacklistShowModal({{$termin->tlid}})" wire:loading.attr="disabled">
                                        {{ __('Dodaj na Blacklistu') }}
                                    </x-jet-button>
                                @endif
