<div class="mt-2 bg-green-100 border-t-4 border-green-500 rounded-b text-green-900 px-4 py-3 shadow-md mb-6" role="alert">
    ...
    @if(!$vrsta_lokacije)
        <div class="my-2 flex justify-around">
            @foreach (App\Models\LokacijaTip::whereIn('id', $tipovi_lokacija)->get() as $value)
                <button wire:click="$set('vrsta_lokacije', {{ $value->id }})" class="flex mt-1 -mb-1 bg-yellow-50 text-sm text-gray-700 uppercase border border-green-500 rounded-md p-1.5 hover:bg-gray-700 hover:text-white" >
                     @switch($value->id)
                        @case(1)
                        ...
                        @break
                    @endswitch
                    <span class="ml-2">{{ $value->lt_naziv }}</span>
                </button>
            @endforeach
        </div>
    @else
        <div class="mt-2">
            <p class="font-bold">{{ App\Models\LokacijaTip::where('id', $vrsta_lokacije)->first()->lt_naziv ?? '---' }}</p>
        </div>
        <div class="mt-2">
                    ...
                    @foreach ($this->lokacijeTipa($vrsta_lokacije) as $value)
                        <tr class="hover:bg-gray-100" wire:click="$emit('novaLokacija', {{ $value->id }})" >  
                                ...
                                <td>
                                    ...
                                    {{ $value->l_naziv }}&nbsp;{{ $value->bl_naziv_sufix }}
                                </td>
                                <td>{{ $value->mesto}}</td>
                                <td>{{ $value->r_naziv}}</td>
                                <td></td>
