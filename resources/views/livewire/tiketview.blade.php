        ...
        <div class="w-2/5 flex-initial mx-2 my-2">
            <div class="bg-sky-100 border-t-4 border-sky-500 rounded-b text-sky-900 px-4 py-3 shadow-md mb-6" role="alert">
                <div class="flex justify-between">
                    ...
                    <div class="flex">
                            <button class="px-2 py-1 max-h-9 bg-white text-sm relative text-gray-600 uppercase border rounded-md hover:bg-gray-700 hover:text-white" wire:click="commentsShowModal({{ $tiket->tremina_lokacijalId }})" title="Komentari">
                                <div class="mx-1 text-lg">{{ $selectedTerminalCommentsCount}}</div>
                            ...
                            <div class="ml-2">
                                <button data-tooltip-target="tooltip-default" type="button" class="mt-1">
                                    <x-heroicon-m-information-circle class="fill-green-500 w-7 h-7" />
                                </button>
            ...
            <div class="border-t-4 border-sky-500 rounded-b text-sky-900 px-4 py-3 shadow-md mb-6" role="alert">
                ...
                <livewire:komponente.terminal-histroy-component :terminal_lokacija_id="$terminal_lokacija_id" />
            </div>
        </div>
        {{-- KOMENTARI --}}
                ...
                <div class="w-full">
                    ...
                    @if($komentari->count())
                        @foreach($komentari as $komentar)
                            <div class="py-2">
                                <div><svg class="float-left fill-slate-600 w-4 h-4 mr-2 mt-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M224 256c70.7 0 128-57.31 128-128s-57.3-128-128-128C153.3 0 96 57.31 96 128S153.3 256 224 256zM274.7 304H173.3C77.61 304 0 381.6 0 477.3c0 19.14 15.52 34.67 34.66 34.67h378.7C432.5 512 448 496.5 448 477.3C448 381.6 370.4 304 274.7 304z"/></svg>
                                <span class="font-bold">{{ $komentar->name }}</span> <span class="text-sm ml-4">{{ App\Http\Helpers::datumFormat($komentar->created_at) }}</span>
                                    <div class="border rounded-lg border-slate-50 bg-slate-200 px-2 py-1 ml-10">{!! nl2br(e($komentar->komentar )) !!}</div>
                                </div>
                            </div>
                        @endforeach
                    @endif
